...
export async function GET(request: Request) {
  try {
    ...
    if (!token) {
      console.error('USER_REVIEWS_FETCH_ERROR — Token tidak ditemukan di header.');
    ...
    if (userError || !userData?.user) {
      console.error('USER_REVIEWS_FETCH_ERROR — Verifikasi token gagal:', userError);
      return NextResponse.json({ error: 'Tidak terautentikasi.' }, { status: 401 });
    }

    const userId = userData.user.id;

    // ── Step 3: Query reviews via service_role ─────────────────────
